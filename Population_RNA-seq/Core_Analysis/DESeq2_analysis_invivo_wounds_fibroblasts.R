# Objective: To compare transcriptomes of in vivo FACS sorted young THY1+ and THY1- fibroblasts before and wounds 7days after recovery using the DESeq2 package
...
setwd('~/Dropbox/Code_checking_SM/For_Brittany/Core_Analysis/Fibroblast_in_vivo_young_wounds//')
...
smCountTable.1a <- HTseq[,grep("Fib_Y1|Fib_Y2|Fib_Y3|Fib_WY", colnames(HTseq))]
...
summary(resSig)
# out of 5393 with nonzero total read count
...
summary(resSIG_FC)
# out of 5206 with nonzero total read count
# adjusted p-value < 0.1
# LFC > 0 (up)       : 2575, 49%
# LFC < 0 (down)     : 2631, 51%
# outliers [1]       : 0, 0%
...
# write tables
write.table(resSIG_FC, file="DESeq2_young_wound_vs_basal_DEGs_1_5FC.csv", sep=',')
write.table(resOrdered, file="DESeq2_young_wound_vs_basal_genes.csv", sep=',')

# save DEG table 
save(resOrdered, file = "DESeq2_DEG_young_wound_vs_basal_all_genes.Rda")
