# Objective: To compare transcriptomes of in vivo FACS sorted young vs old THY1+ and THY1- fibroblasts in wounds 7days after recovery using the DESeq2 package
...
setwd('~/Dropbox/Code_checking_SM/For_Brittany/Core_Analysis/Fibroblast_invivo_young_vs_old_wounds/')
...
smCountTable.1a <- HTseq[,grep("Fib_WO|Fib_WY", colnames(HTseq))]
colnames(smCountTable.1a)

# reorder so that young (3m) comes first
smCountTable.1a <- smCountTable.1a[,c(grep("Fib_WY", colnames(smCountTable.1a)),
                                      grep("Fib_WO", colnames(smCountTable.1a)))]
colnames(smCountTable.1a)
...
smCountTable.filt <- smCountTable.1[keep,] # maintain 15382 genes for further analysis

# specify condtion, batch and cell type (if they are THY1pos or THY1neg)
condition = c(rep("young",5),
              rep("old",6))
celltype <- factor(c("Neg","Pos","Neg","Pos","Pos",
...
resultsNames(dds)
# [1] "Intercept"              "celltype_Pos_vs_Neg"    "condition_young_vs_old"
...
summary(resSig)
# out of 49 with nonzero total read count
...
# write tables
write.table(resSIG_FC, file="DESeq2_young_vs_old_wound_DEGs_1_5FC.csv", sep=',')
write.table(resOrdered, file="DESeq2_young_vs_old_wound_genes.csv", sep=',')

# save DEG table 
save(resOrdered, file = "DESeq2_DEG_young_vs_old_wound_all_genes.Rda")
...
# save VST table 
save(vstMat, file = "DESeq2_young_vs_old_wound_all_vst_values.Rda")
