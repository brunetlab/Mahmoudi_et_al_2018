...
setwd('~/Dropbox/Code_checking_SM/For_Brittany/Core_Analysis/Fibroblast_invivo_all_data/')
...
smCountTable.1a <- HTseq[,grep("Fib_O|Fib_Y|Fib_WO|Fib_WY", colnames(HTseq))]
...
smCountTable.filt <- smCountTable.1[keep,] # maintain 17271 genes for further analysis

# specify condtion, batch and cell type (if they are THY1pos or THY1neg)
condition = c(rep("basal",21),
              rep("wound",11))

batch = factor(c(rep("a",6),
               rep("b",5),
               rep("a",4),
               rep("b",6),
               rep("a",11)))
...
resultsNames(dds)
#[1] "Intercept"                "batch_b_vs_a"             "condition_wound_vs_basal"
...
summary(resSig)
# out of 6478 with nonzero total read count
...
summary(resSIG_FC)
# out of 5824 with nonzero total read count
# adjusted p-value < 0.1
# LFC > 0 (up)       : 2839, 49%
# LFC < 0 (down)     : 2985, 51%
# outliers [1]       : 0, 0%
...
# save DEG table 
save(resOrdered, file = "DESeq2_DEG_invivo_all_Data_all_genes.Rda")
...
# save VST table 
save(v.data, file = "DESeq2_invivi_all_Data_all_vst_values.Rda")
