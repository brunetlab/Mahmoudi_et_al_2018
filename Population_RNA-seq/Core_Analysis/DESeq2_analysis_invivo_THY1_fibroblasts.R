...
setwd('~/Dropbox/Code_checking_SM/For_Brittany/Core_Analysis/THY1_Fibroblast_in_vivo/')
...
res <- results(dds, name = "celltype_Pos_vs_Neg", cooksCutoff=T, independentFiltering = T)
...
summary(resSig)
# out of 703 with nonzero total read count
...
summary(resSIG_FC)
# out of 597 with nonzero total read count
# adjusted p-value < 0.1
# LFC > 0 (up)       : 406, 68%
# LFC < 0 (down)     : 191, 32%
# outliers [1]       : 0, 0%
...
# write tables
write.table(resSIG_FC, file="DESeq2_THY1_in_vivo_DEGs_1_5FC.csv", sep=',')
write.table(resOrdered, file="DESeq2_THY1_in_vivo_all_genes.csv", sep=',')

# save DEG table 
save(resOrdered, file = "DESeq2_DEG_THY1_in_vivo_all_genes.Rda")
...
# save VST table 
save(v.data, file = "DESeq2_THY1_in_vivo_vst_values.Rda")
